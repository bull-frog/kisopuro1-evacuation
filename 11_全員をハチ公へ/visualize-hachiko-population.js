...
for (let t = 0; t < 200; t++) {
	...
	// シミュレーションの記録を格納すべし
	hachikoPopulation.push(agentsInNodes[187].length);
	// console.log(`Population at Hachiko Square: ${agentsInNodes[187].length}`);

}

// シミュレーションの記録を出力する
const csv = hachikoPopulation.map((value, index) => `${index * timeInterval},${value}`).join("\n");
console.log(csv);
fs.writeFileSync("hachikoPopulation.csv", "time,population\n" + csv);