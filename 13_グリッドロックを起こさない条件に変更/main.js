 ...
 * 【シミュレーションの条件】
 * - 初期の人口分布は均一
 * - 目的地は、ハチ公に向かう人の割合をいくつか設定、他は代々木公園に向かう
 */
...
const Agent = require("./agent-hachiko-yoyogi").Agent;
const generateAgents = require("./agent-hachiko-yoyogi").generateAgents;
...
for (let i = 0; i < nodes.length; i++) {
	const dijkstraResult = dijkstra(linksWithDistances, nodes.length, i)
	routes.push(dijkstraResult.nextPointOnRoute);
	distances.push(dijkstraResult.distances);
}

// エージェントの初期化
let agents = generateAgents(linksWithDistances, routes, distances);
